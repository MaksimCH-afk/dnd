...
Сверху: **📚** кампании (с сервера; там же «Импорт из документов» — собрать партию
из листа персонажа/инвентаря/NPC/описания сессии), **💳** справка по платным моделям
OpenRouter на роль Ведущего, **⚙** настройки (включая раздел «Администрирование» —
правка ключей и моделей по ролям под паролем). Индикатор ⊙ — связь с сервером.
