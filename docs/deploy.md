...
2. **Через админ-панель в браузере — `.env` не нужен.** ⚙ Настройки → «Администрирование».
   При первом входе клиент попросит **задать пароль** администратора (хранится в БД хешем).
   Дальше под ним правятся ключи и модели по ролям — всё сохраняется в БД и применяется
   сразу, без рестарта и без файлов. Секреты не возвращаются в браузер. Без TLS пароль идёт
   открытым — для публичного доступа используйте Caddy+TLS или Tailscale.

   То есть после `docker compose up -d --build` можно вообще не трогать `.env`: открыть игру,
   задать пароль админа, вписать ключ OpenRouter и модели — и играть.
