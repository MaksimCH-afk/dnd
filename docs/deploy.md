...
2. **Через админ-панель в браузере — `.env` не нужен.** ⚙ Настройки → «Администрирование» →
   вписать ключ OpenRouter и (опц.) модели по ролям → «Сохранить». Всё хранится в БД и
   применяется сразу, без рестарта и без файлов. Секреты не возвращаются в браузер.

   То есть после `docker compose up -d --build` можно вообще не трогать `.env`: открыть игру,
   вписать ключ OpenRouter и модели — и играть.

   > У админки **нет отдельного пароля** — она открыта тому, кто имеет доступ к серверу.
   > Поэтому **ограничивайте доступ на уровне сети**: Tailscale (приватно) или обратный
   > прокси с авторизацией (Caddy basic-auth) для публичного адреса. Не открывайте 8787
   > в интернет без этого.
