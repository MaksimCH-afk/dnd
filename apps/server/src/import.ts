	...
	for (let attempt = 0; attempt < 2 && !p; attempt++) {
		...
		if (!raw.trim()) {
			if (r.error && /\b429\b/.test(r.error)) {
				lastErr = 'OpenRouter: лимит бесплатных моделей (429 Too Many Requests). Подождите минуту и повторите, либо пополните баланс OpenRouter (≥$10 поднимает дневной лимит free-моделей), либо выберите платную модель Ведущего в ⚙ Администрирование.';
			} else if (r.error) {
