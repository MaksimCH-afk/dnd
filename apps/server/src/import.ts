	...
	for (let attempt = 0; attempt < 2 && !p; attempt++) {
		...
		if (!raw.trim()) {
			if (r.error && /\b429\b/.test(r.error)) {
				lastErr = 'OpenRouter: лимит запросов к free-моделям (429 Too Many Requests). Подождите ~минуту и повторите. Если повторяется — выбранные бесплатные модели сейчас перегружены; выберите платную модель Ведущего в ⚙ Администрирование.';
			} else if (r.error) {
				lastErr = `модель Ведущего недоступна: ${r.error}. Проверьте id модели/ключ в ⚙ Администрирование.`;
			} else {
				lastErr = 'модель Ведущего вернула пустой ответ (проверьте ключ OpenRouter и лимиты модели)';
			}
			break; // не долбим повторно при ошибке/пустом ответе — это и провоцирует 429
		}
		try {
			p = extractJson(raw);
		} catch (e) {
